...
@dataclass
class SchedulePickupResult:
    success: bool
    message: str
    conflict_open_order: bool = False
...
def schedule_fresh_pickup(mobile: str, pickup_date: str, pickup_time: int) -> SchedulePickupResult:
    payload = {
        "mobile": mobile,
    ...
    try:
        ...
        logger.info(
            "schedule_fresh_pickup request mobile=%s date=%s time=%s status_code=%s",
            _mask_mobile(mobile),
            pickup_date,
            pickup_time,
            status_code,
        ...
        logger.exception(
            "schedule_fresh_pickup exception mobile=%s error=%s",
            _mask_mobile(mobile),
            error,
        )
        return SchedulePickupResult(
